...
public interface WorkerRepository extends ListCrudRepository<Worker, String> {
    ...
    /** Returns all ACTIVE workers grouped by queue, used for queue stats aggregation. */
     ...
     * Only transitions ACTIVE → STALE (does not touch already-STALE or DEAD workers).
     */
    @Modifying
    @Query("""
            UPDATE workers
