...
public interface TaskRepository extends ListCrudRepository<Task, UUID> {
   ...
   * Returns the affected tasks so callers can fire webhooks for those with
   * callback URLs.
  ...
  /**
   * Replays a single DEAD task back to PENDING. Resets attempts to 0, clears
   * worker_id, claimed_at, finished_at, and next_attempt_at. Preserves
   * last_error.
   * The WHERE status = 'DEAD' guard ensures only one concurrent replay succeeds.
   * Returns the updated task, or an empty list if the task is not DEAD.
   */
  @Query("""
      UPDATE tasks
         SET status          = 'PENDING',
             ...
             deadline        = CASE
                                 WHEN CAST(:deadline AS TIMESTAMPTZ) IS NOT NULL THEN CAST(:deadline AS TIMESTAMPTZ)
                                 ELSE deadline
                               END
       WHERE id     = :taskId
         AND status = 'DEAD'
      RETURNING *
      """)
  List<Task> replayTask(@Param("taskId") UUID taskId,
      @Param("maxAttempts") int maxAttempts,
      @Param("deadline") Instant deadline);

  /**
   * Replays all DEAD tasks in a given queue back to PENDING. Optionally filters
   * by {@code finished_at} to only replay recently dead-lettered tasks.
  ...
  @Query("""
      UPDATE tasks
         SET status          = 'PENDING',
             attempts        = 0,
             max_attempts    = CASE
                                 WHEN :maxAttempts > 0 THEN :maxAttempts
                                 ELSE max_attempts
                               END,
             worker_id       = NULL,
             claimed_at      = NULL,
       ...
       WHERE queue_name = :queueName
         AND status     = 'DEAD'
         AND (CAST(:deadSince AS TIMESTAMPTZ) IS NULL OR finished_at >= CAST(:deadSince AS TIMESTAMPTZ))
      """)
  int bulkReplayTasks(@Param("queueName") String queueName,
